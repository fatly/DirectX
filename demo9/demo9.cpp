...
HRESULT InitObject(void)
{
	...
	_tcscat_s(szFileName, TEXT("lighting.X"));
	...
	//创建一个极薄的屏障
	D3DXCreateBox(g_pDevice, 120.0f, 120.0f, 0.3f, &g_pMeshWall, NULL);
...
void SetMatrix(void)
{
	...
	//取景变换矩阵的设置  
	D3DXMATRIX matView;
	D3DXVECTOR3 eye(100.0f, 0.0f, -250.0f);
...
void Render(HWND hWnd)
{
	g_pDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER | D3DCLEAR_STENCIL, D3DCOLOR_XRGB(100, 150, 0), 1.0f, 0);

	g_pDevice->BeginScene();

	D3DXMATRIX matHore, matWorld, matRotation;

	D3DXMatrixTranslation(&matHore, -20.0f, 0.0f, -25.0f);
	matHore = matHore * g_matWorld;
	g_pDevice->SetTransform(D3DTS_WORLD, &matHore);
	...
	//重新设置一系列渲染状态，将镜像与镜面进行融合运算，并清理一下Z缓存  
	...
	_stprintf_s(strInfo, -1, TEXT("模型坐标:(%0.2f, %0.2f, %0.2f)"), matHore._41, matHore._42, matHore._43);
