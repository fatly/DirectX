...
HRESULT InitObject(void)
{
	...
	_tcscat_s(szFileName, TEXT("bee.X"));
	...
	D3DXCreateCylinder(g_pDevice, 4000.0f, 100.0f, 50000.0f, 60, 60, &g_pCylinder, 0);
	...
	sprintf_s(szTexName, "%s%s", szTexPath, "green.jpg");
	g_pTerrain = new Terrain(g_pDevice);
	g_pTerrain->Load(szRawFile, szTexName);
	g_pTerrain->Initialize(200, 200, 500.0f, 60.0f);
...
void Update(HWND hWnd)
{
	...
	if (g_pInputs->IsKeyDown(DIK_A)) g_pCamera->MoveRight(-10.0f);
	if (g_pInputs->IsKeyDown(DIK_D)) g_pCamera->MoveRight(10.0f);
	if (g_pInputs->IsKeyDown(DIK_W)) g_pCamera->MoveLook(10.0f);
	if (g_pInputs->IsKeyDown(DIK_S)) g_pCamera->MoveLook(-10.0f);
	if (g_pInputs->IsKeyDown(DIK_I)) g_pCamera->MoveUp(10.0f);
	if (g_pInputs->IsKeyDown(DIK_K)) g_pCamera->MoveUp(-10.0f);
...
void Render(HWND hWnd)
{
	g_pDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER | D3DCLEAR_STENCIL, D3DCOLOR_XRGB(0, 100, 255), 1.0f, 0);

	g_pDevice->BeginScene();

	//绘制大黄蜂
	D3DXMATRIX scaleMatrix, rotMatrix1, rotMatrix2, tranMatrix, finalMatrix;
	D3DXMatrixScaling(&scaleMatrix, 20.0f, 20.0f, 20.0f);
	D3DXMatrixTranslation(&tranMatrix, 0.0f, 8000.0f, 0.0f);
	D3DXMatrixRotationX(&rotMatrix1, D3DX_PI / 2.0f);
	D3DXMatrixRotationY(&rotMatrix2, D3DX_PI / 2.0f);
	finalMatrix = scaleMatrix * rotMatrix1 * rotMatrix2 * tranMatrix * g_matWorld;
	...
	for (int i = 0; i < 4; i++)
	{
		D3DXMatrixTranslation(&tranMatrix1, -10000.0f, 0.0f, -15000.0f + (i * 20000.0f));
