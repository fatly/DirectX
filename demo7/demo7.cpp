...
D3DMATERIAL9*			g_pMaterials = 0;
LPDIRECT3DTEXTURE9*		g_pTextures = 0;
DWORD					g_dwMaterialCount = 0;
...
HRESULT InitObject(void)
{
	...
	_tcscat_s(szFileName, TEXT("miki.X"));
...
void Render(HWND hWnd)
{
	...
	rect.top += 25;
	g_pTextHelper->DrawText(NULL, TEXT("    键盘上1,2,3,4数字键：在四种寻址模式之间切换"), -1, &rect, DT_SINGLELINE | DT_NOCLIP | DT_LEFT, D3DCOLOR_RGBA(255, 200, 0, 255));
	rect.top += 25;
